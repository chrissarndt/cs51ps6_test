...
let rec list_to_stream (lst : obj list) : event NLS.stream =
  let rec list_to_stream_rec nlst =
    ...
    | [] -> list_to_stream lst
    | Note(pit, tim, vol) :: t-> 
      lazy(Cons(Tone(0., pit, vol), lazy(Cons(Stop(tim, pit), list_to_stream_rec t))))
    | Rest(tim) :: t -> shift_start tim (list_to_stream_rec t)
...
let rec pair (a : event NLS.stream) (b : event NLS.stream)
           : event NLS.stream =
  let NLS.Cons(h1, t1), NLS.Cons(h2, t2) = Lazy.force a, Lazy.force b in 
  let at, bt = time_of_event h1, time_of_event h2 in 
  if at < bt then 
    lazy(NLS.Cons(h1, pair t1 (shift_start (0. -. at) b)))
  else 
    lazy(NLS.Cons(h2, pair (shift_start (0. -. bt) a) t2));;
...
let transpose (str : event NLS.stream) (half_steps : int)
  ...
  NLS.smap 
    (fun ev -> match ev with 
               | Tone(tim, pit, vol) -> 
                   Tone(tim, (transpose_pitch pit half_steps), vol)
               | Stop(tim, pit) -> Stop(tim, transpose_pitch pit half_steps))
    str;;
